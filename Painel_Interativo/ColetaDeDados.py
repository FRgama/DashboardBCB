...
#Selecionando Dados
def ColetandoDados():
    dfSelic = sgs.get({'Selic': 1178}, start='2016-01-01')
    dfSelic = pd.DataFrame(dfSelic)

    dfIpca = sgs.get({'Ipca': 4449}, start='2016-01-01')
    dfIpca = pd.DataFrame(dfIpca)

    dfSMin = sgs.get({'Salario_Minimo': 1619}, start='2016-01-01')
    dfSMin = pd.DataFrame(dfSMin)

    dfIgpm = sgs.get({'Igpm': 189}, start='2016-01-01')
    dfIgpm = pd.DataFrame(dfIgpm)

    dfInad = sgs.get({'Inadimplencia': 21082}, start='2016-01-01')
    dfInad = pd.DataFrame(dfInad)

    dfInadFamiliar = sgs.get({'Inadimplencia Familiar': 29038}, start='2016-01-01')
    dfInadFamiliar = pd.DataFrame(dfInadFamiliar)
    ...
    dfInadFamiliar = ajusteColunas(dfInadFamiliar, 'Inadimplencia Familiar')
    ...
    dfInadFamiliar = ajusteData(dfInadFamiliar)
    ...
    dfInadFamiliar = variacao(dfInadFamiliar, 'Inadimplencia Familiar')
    ...
    dfInadFamiliar = dfInadFamiliar.rename(columns={'Variação': 'Variacao_InadFamiliar'})
    ...
    df_geral = dfSelic.merge(dfIpca, on='Data', how='inner')\
                    ...
                    .merge(dfInadFamiliar, on='Data', how='inner')
    ...
    dfInadFamiliar = arredondarColuna(dfInadFamiliar, 'Variacao_InadFamiliar')
    ...
    print(dfInad.tail())
    print(df_indicadores.tail())
    return dfIpca, dfSelic, dfSMin, dfIgpm, dfInad, dfInadFamiliar